...
private:
    ...
    const float AIR_ACCEL = 1.0f; 
    const float JUMP_SPEED = 8.0f;
    const float GRAVITY = 20.0f;
    const float GROUND_Y = 0.5f;
    const float MAX_SPEED = 50.0f;
    const float STRAFE_ACCEL = 1.5f;
    const float BUNNY_HOP_SPEED_RETAIN = 0.98f; 
    const float COYOTE_TIME = 0.1f;
    const float QUAKE_UNITS_SCALE = 20.0f;
    const float AIR_ACCEL_CAP = 30.0f;
    const float WALL_SURF_ACCEL = 5.0f;
    const float WALL_DETECTION_DISTANCE = 1.2f;
    ...
    WallInfo CheckWallCollision(const glm::vec3& direction);
    void HandleWallSurfing(const WallInfo& wallInfo, float deltaTime);
...
public:
    ...
    float GetSpeedUPS() const {
        return GetHorizontalSpeed() * QUAKE_UNITS_SCALE;
    }
